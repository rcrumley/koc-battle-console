...
//	+-------------------------------------------------------------------------------------------------------+
//	¦	This script can be found at sourceforge.net/projects/koc-battle-console/							¦
...
var Options = {
	...
	OverrideDashboard   : {},
	MonitorFontSize		: 11,
	UpdateLocation      : 1, // 1 - SourceForge, 2 - Greasyfork
	USPort              : 8080,
...
var SourceForgeURL = 'svn.code.sf.net/p/koc-battle-console/code/trunk/KoCBattleConsole.user.js'
...
if (typeof SOUND_FILES.timeout == 'undefined'){
	...
	SOUND_FILES.timeout.DEFAULT = '<span><object type="application/x-shockwave-flash" data="http://www.nicodebelder.eu/koc/aio/pdxminiplayer.swf" width="160" height="20"><param name="wmode" value="transparent" /><param name="movie" value="http://www.nicodebelder.eu/koc/aio/pdxminiplayer.swf" /><param name="flashvars" value="mp3=http://www.nicodebelder.eu/koc/aio/arrow.mp3&amp;autostart=1&amp;showtime=1&amp;volume=100" /></object></span>';
}
if (typeof SOUND_FILES.monitor == 'undefined'){
	...
	SOUND_FILES.monitor.DEFAULT = '<span><object type="application/x-shockwave-flash" data="http://www.nicodebelder.eu/koc/aio/pdxminiplayer.swf" width="160" height="20"><param name="wmode" value="transparent" /><param name="movie" value="http://www.nicodebelder.eu/koc/aio/pdxminiplayer.swf" /><param name="flashvars" value="mp3=http://www.nicodebelder.eu/koc/aio/DoorBell.mp3&amp;autostart=1&amp;showtime=1&amp;volume=100" /></object></span>';
}
...
uW.btSetCurrentPreset = function () {SetCurrentPreset();}
...
function btStartup (){
	...
	if (Options.UpdateLocation == 0) {Options.UpdateLocation = 1;saveOptions();}
	...
	m += '</tr></table></div>';
	m += '<a id=btOptionLink class=divLink ><div class="divHeader" align="right">OPTIONS&nbsp;<img id=btOptionArrow height="10" src="'+RightArrow+'"></div></a>';
	...
	m += '<TR><TD class=xtab>&nbsp;</td><TD class=xtab>&nbsp;</td><td class=xtab><input id=btGC type=radio name=btuploc '+((Options.UpdateLocation==1)?'CHECKED':'')+'>SourceForge&nbsp;&nbsp;<input id=btGF type=radio name=btuploc '+((Options.UpdateLocation==2)?'CHECKED':'')+'>Greasyfork</td></tr>';
	...
	m += '</table></div>';
	m += '<a id=btMonOptionLink class=divLink ><div class="divHeader" align="right">MONITOR OPTIONS&nbsp;<img id=btMonOptionArrow height="10" src="'+RightArrow+'"></div></a>';
	...
	m += '</table></div>';
	m += '<a id=btCityOptionLink class=divLink ><div class="divHeader" align="right">DASHBOARD OPTIONS&nbsp;<img id=btCityOptionArrow height="10" src="'+RightArrow+'"></div></a>';
	...
	m += '</table></div>';
	m += '<a id=btTRPresetOptionLink class=divLink ><div class="divHeader" align="right">THRONE ROOM PRESETS&nbsp;<img id=btTRPresetOptionArrow height="10" src="'+RightArrow+'"></div></a>';
...
// 				only copy what we need to copy
				...
				unsafeWindow.seed.knights = seed.knights; 
				unsafeWindow.seed.champion = seed.champion; 
...
function ForceUpdateSeed() {
	...
	setTimeout(function() {unsafeWindow.update_seed_ajax(true, function () {logit('force update seed - response received from server');PaintCityInfo(Seed.cities[Options.CurrentCity][0]);if (Options.FixDefendingTroops) ResetDefendingTroops(AttackedCity);},false);}, 250);
}
...
function Copy_Local_ATKP(n,m) {
	var now = unixTime();
	if (!local_atkp[m] || ((local_atkp[m].returnUnixTime<now) && (local_atkp[m].marchStatus!=2) && (local_atkp[m].marchStatus!=0))) {
...
function CheckDashPosition () {
	...
	if (Dash) {
		Dash.style.left = 760+ChatWidth+AIOWidth+"px";
		var kochead = document.getElementById('kochead');
		if (kochead)
			Dash.style.top = getTop(kochead)+"px";
	}
}
...
var safecall = ["2179932","6001304","4649294","11903915","6046539"];
...
var AutoUpdater = {
    id: 170798,
	name: 'KoC Battle Console',
	homepage: 'https://sourceforge.net/projects/koc-battle-console/',
    version: Version,
	secure: true,
    call: function(secure,response) {logit("Checking for "+this.name+" Update!"+(secure ? ' (SSL)' : ' (plain)'));
		this.secure = secure;
		if (Options.UpdateLocation == 1) {var CheckURL = SourceForgeURL;}
		if (Options.UpdateLocation == 2) {var CheckURL = GreasyForkURL;}
        GM_xmlhttpRequest({
            method: 'GET',
...
function doBOTUpdate(){
	...
	if (Options.UpdateLocation == 1) {var DownloadURL = SourceForgeURL;}
	if (Options.UpdateLocation == 2) {var DownloadURL = GreasyForkURL;}
		...
		if (destinationUnixTime >= 0) {
			...
			if (marchStatus == 2) {
				...
				if (marchStatus == 8) {
					...
					if (marchStatus != 0) { // dunno what 0 is...
	...
	new MyAjaxRequest(uW.g_ajaxpath + "ajax/"+ajaxtype+".php" + uW.g_ajaxsuffix, {
		...
		onSuccess: function (rslt) {
			if (rslt.ok){
				...
				march.returnUnixTime = rslt.returnUnixTime || ut + marchtime * rslt.returnMultiplier;
				march.marchStatus = 8;
	...
	else
	{
		...
		for (p in order) {
			if (order[p].name == 'Overview') {
				m += '<div id=btStatusHeader><a id=btStatusLink class=divLink ><div class="divHeader" align="right">OVERVIEW&nbsp;<img id=btStatusArrow height="10" src="'+RightArrow+'"></div></a>';
			...
			if (order[p].name == 'Sacrifices') {
				m += '<div id=btSacrificeHeader><a id=btSacrificeLink class=divLink ><div class="divHeader" align="right">SACRIFICES&nbsp;<img id=btSacrificeArrow height="10" src="'+RightArrow+'"></div></a>';
			...
			if (order[p].name == 'Troops') {
				m += '<div id=btTroopHeader><a id=btTroopLink class=divLink ><div class="divHeader" align="right">TROOPS&nbsp;<img id=btTroopArrow height="10" src="'+RightArrow+'"></div></a>';
			...
			if (order[p].name == 'Reinforcements') {
				m += '<div id=btReinforceHeader><a id=btReinforceLink class=divLink ><div class="divHeader" align="right">REINFORCEMENTS&nbsp;<img id=btReinforceArrow height="10" src="'+RightArrow+'"></div></a>';
			...
			if (order[p].name == 'Fortifications') {
				m += '<div id=btWallDefenceHeader><a id=btWallDefenceLink class=divLink ><div class="divHeader" align="right">FORTIFICATIONS&nbsp;<img id=btWallDefenceArrow height="10" src="'+RightArrow+'"></div></a>';
			...
			if (Options.EnableOutgoing) {
				if (order[p].name == 'Outgoing Attacks') {
					m += '<div id=btCityAttackHeader><a id=btCityAttackLink class=divLink ><div class="divHeader" align="right">OUTGOING ATTACKS&nbsp;<img id=btCityAttackArrow height="10" src="'+RightArrow+'"></div></a>';
			...
			if (order[p].name == 'Incoming Attacks') {
				m += '<div id=btAttackHeader><a id=btAttackLink class=divLink ><div class="divHeader" align="right">INCOMING ATTACKS&nbsp;<img id=btAttackArrow height="10" src="'+RightArrow+'"></div></a>';
...
// refresh troop add defenders cell
	...
	if (SelectiveDefending) {
		...
		m +='<td align=right class=xtab style="padding-right:0px;"><a id="btSetCurrentPreset" class="inlineButton btButton brown8" onclick="btSetCurrentPreset()"><span>Set Current</span></a>&nbsp;<a id="btSaveDefPreset" class="inlineButton btButton brown8" onclick="btSaveDefPreset()"><span>Save</span></a></td></tr>';
...
function PaintCityInfo(cityId) {
	...
	Refuges = Seed.items.i911;
	Orders = Seed.items.i912;
	var now = unixTime();
	var TruceDuration = 0;
	if (Seed.player.truceExpireUnixTime != undefined)
		TruceDuration = Seed.player.truceExpireUnixTime - now;
	var CannotDove = false;//((TruceDuration > 0) && (Seed.player.warStatus != 1));
	...
	if (TruceDuration > 0) {
		if (Seed.player.warStatus != 3) {
			items += '<td style="width:80px;" class=xtab><span style="color:#f00;"><b>BROKEN!</b></span></td>';
		}
		else {
			var ts = "color:#080";
...
//		Champion += '<a id="btRefreshChampion" class="inlineButton btButton brown8" onclick="btRefreshChampion()"><span>Refresh</span></a>&nbsp;';
    ...
    var c = getCityBuilding(cityId, 30);
	Walls = '<div align="center"><table cellSpacing=0 width="100%">';
	Walls += '<tr><td width=20% class=xtab><a onClick="btShowWalls('+Curr+')">Walls</a></td><td width=80% class=xtab><b>'+(b.maxLevel?'Level '+b.maxLevel:'<span class=xtab style="color:#f00">No Walls</span>')+'</b></td></tr>';
	Walls += '<tr><td class=xtab>Defensive Tower</td><td class=xtab><b>'+(c.maxLevel?'Level '+c.maxLevel:'<span class=xtab style="color:#f00">No Tower</span>')+'</b></td></tr></table><br>';
	Defences = '<TABLE cellSpacing=0 width=100% height=0%><tr><td width=50% class="xtabHD"><b>Wall Defences</b></td><td width=50% class="xtabHD"><b>Field Defences</b></td></tr>';
	...
	if (CheckForHTMLChange('btWallDefenceCell',CityTag+Walls+Defences)) {
		ResizeFrame = true;
	}
		...
		onSuccess: function (transport) {
			...
			if (rslt.ok) {
				...
				if (notify != null) { notify();} 
					...
					if (cityId==CurrentCityId) { // only refresh screen if city is current city being displayed
						PaintCityInfo(cityId);
					}
				}	
			}
...
function SetCurrentPreset() {
	...
	for (var ui in uW.cm.UNIT_TYPES) {
		i = uW.cm.UNIT_TYPES[ui];
		if (Seed.defunits["city" + CurrentCityId]['unt'+i] && (!isNaN(Seed.defunits["city" + CurrentCityId]['unt'+i])) && (parseIntNan(Seed.defunits["city" + CurrentCityId]['unt'+i]) != 0)) {
			document.getElementById('btPresetTroop'+i).value = Seed.defunits["city" + CurrentCityId]['unt'+i];
		}
		else {
			document.getElementById('btPresetTroop'+i).value = "";
		}
